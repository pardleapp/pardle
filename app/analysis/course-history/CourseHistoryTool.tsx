...
export default function CourseHistoryTool() {
  ...
  // rounds threshold changes. Drift filtering happens client-side, so
  // toggling it doesn't refetch. The API returns the same rows the
  // By-course tab draws from — no server-side hiding.
  useEffect(() => {
  ...
  return (
    ...
    >
      <SectionHeader
        ...
        subtitle={
          total
            ? `${total.toLocaleString()} clean (player, course) pairs · ranked by outperformance vs baseline`
      ...
      >
        ...
        Baselines are Bayesian-shrunk toward tour average so low-sample
        DP/opposite-field entries don&apos;t distort the top. Players
        whose current tour form has drifted more than 1.0 SG from their
        historical baseline (recent breakouts or declines) are removed
        entirely — those numbers reflect a skill change, not course
        fit. The same clean set feeds the By-course tab.
      </div>
